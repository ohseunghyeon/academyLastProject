					...
					<fieldset id="content">
						...
						<fieldset>
							...
							<table
								...
								id="payment" cellspacing="10" cellpadding="0">
								<tr id="tt">
								...
								<c:forEach var="foodDto" items="${shoppingBagForPayment}">
									<input type="hidden" name="food_code"
										value="${foodDto.food_code}">
									<input type="hidden" name="food_num"
										value="${foodDto.food_num}">
									<tr>
						...
						<fieldset>
							...
							<input type="text" name="couponMileageUse"
								ng-model="couponMileageUse" value="0" ng-hide="true">

							<c:if test="${coupon == 0 && mileage == 0}">
								<p>사용 가능한 마일리지나 쿠폰이 없습니다.</p>
							</c:if>
							<c:if test="${coupon != 0 || mileage != 0}">
								<span id="badge" class="badge" name="selectsale"
									ng-click="nouse()">미사용</span>
							</c:if>
							<c:if test="${coupon != 0}">
								<span id="badge" class="badge" name="selectsale"
									ng-click="coupon()">쿠폰조회</span>
							</c:if>
							<c:if test="${mileage != 0}">
								<span id="badge" class="badge" name="selectsale"
									ng-click="mileage()">마일리지사용</span>
							</c:if>

							<table
								class="table table-bordered table-hover table-condensed text-center"
								id="payment" cellspacing="5">
								<tr ng-hide="couponHide">
									<th id="tt" width="200" height="15"><p class="text-center">보유
											쿠폰 할인</p></th>
									<td><input type="text" name="coupon" size="20"
										placeholder="Enter coupon number" />
										<button class="btn btn-warning" type="button" name="coupon1"
											onclick="couponPage()">쿠폰 조회</button></td>
								</tr>
						...
						<fieldset>
							...
							<table
								...
								id="payment" cellspacing="5">
								<tr>
									<th id="tt" width="200" height="15"><p class="text-center">최종
											결제 금액</p></th>
									<td><mark>
											<input type="text" readonly name="finalamount" size="10"
												value="${price}">
										</mark>원</td>
						...
						<p class="text-center">
							...
							<button type="button" onclick="window.location='goodsList'"
								class="btn btn-warning">결제 취소</button>
						</p>
					</fieldset>
				</form>
			</div>
