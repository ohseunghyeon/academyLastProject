 ...
 * 	브라우저가 했던 서버와의 통신부분을 자바스크립트 조작을 통해 가능하다.
 * 	브라우저 표준 오브젝트의 하나지만 버전별로 차이가 생긴다.
 * 	Microsoft.XMLHTTP는 익스플로어 오브젝트이고,
...
function sendRequest(callback, url, method, params) {
	...
	// onreadystatechange : 오브젝트 상태가 변한 시점에 불러오는 이벤트 핸들러
	// callback 함수 :
	// 즉 대기상태가 바뀌면 실행할 이벤트라는 의미 .. 서버로부터 응답이 오면 동잘할 콜백함수(시스템이 자동호출)
